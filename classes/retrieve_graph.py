...
class HydraRetrieveGraph(HydraRetrieve):
    ...
    def morph(self, routine:str, csv_predicates:list = None):
        ...
        # Routine: to CSV
        if routine == 'to-csv':
            progress_message = 'Producing CSV table from graph data'
            self.report.echo_progress(progress_message, 0, 100)
            status = self.__morph_to_csv(csv_predicates)
            self.report.echo_progress(progress_message, 100, 100)

        # Routine: CGIF to NFDI
        elif routine == 'cgif-to-nfdi':
            progress_message = 'Converting graph data to NFDI style'
            self.report.echo_progress(progress_message, 0, 100)
            status = self.__morph_cgif_to_nfdi()
            self.report.echo_progress(progress_message, 100, 100)
    ...
    def __morph_to_csv(self, csv_predicates:list = None) -> dict:
        '''
    ...
    def __morph_cgif_to_nfdi(self) -> dict:
        '''
    ...
    def save(self, target_folder_path:str, file_name:str, routine:str = None):
        ...
        # Routine: CSV
        if routine == 'csv':
            progress_message = 'Saving CSV data'
            self.report.echo_progress(progress_message, 0, 100)
            status = self.__save_csv(file_path)
            self.report.echo_progress(progress_message, 100, 100)

        # Routine: NFDI-style triples
        elif routine == 'nfdi':
            progress_message = 'Saving NFDI-style triples'
            self.report.echo_progress(progress_message, 0, 100)
            status = self.__save_nfdi(file_path)
            self.report.echo_progress(progress_message, 100, 100)

        # Routine: triples
        elif routine == 'triples':
            progress_message = 'Saving triples'
            self.report.echo_progress(progress_message, 0, 100)
            status = self.__save_triples(file_path)
            self.report.echo_progress(progress_message, 100, 100)
    ...
    def __save_csv(self, file_path:str) -> dict:
        '''
    ...
    def __save_nfdi(self, file_path:str) -> dict:
        '''
        Save tabular CSV to file
    ...
    def __save_triples(self, file_path:str) -> dict:
        '''
        Save tabular CSV to file
